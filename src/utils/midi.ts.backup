...
export class MIDIManager {
  ...
  private scanForDevices() {
    ...
    WebMidi.inputs.forEach((input) => {
      ...
      const matchingOutput = WebMidi.outputs.find(
        (output) => output.name === input.name || output.id === input.id.replace("input", "output")
      );
  ...
  private isMonologue(deviceName: string): boolean {
    const monologueNames = ["monologue", "korg monologue", "monologue SOUND"];
    return monologueNames.some((name) => deviceName.toLowerCase().includes(name.toLowerCase()));
  }
  ...
  private parseSysEx(data: Uint8Array): MonologuePatch | null {
    try {
      ...
      if (
        command === MONOLOGUE_SYSEX.COMMANDS.CURRENT_PROGRAM_DATA_DUMP ||
  ...
  getMonologueDevice(): MIDIDevice | null {
    return this.connectedDevices.find((device) => this.isMonologue(device.name)) || null;
  }
